<img src="isicloud5gb.gif" alt="isicloud5gb.lol" width="56">

# [isicloud5gb.lol](https://isicloud5gb.lol) - Is iCloud's Free tier still 5GB?

**Да.**

[![X (Twitter)](https://img.shields.io/badge/X%20%2F%20Twitter-000000?style=for-the-badge&logo=x&logoColor=white)](https://twitter.com/isicloud5gb)
[![Bluesky](https://img.shields.io/badge/Bluesky-0285FF?style=for-the-badge&logo=bluesky&logoColor=white)](https://bsky.app/profile/isicloud5gb.lol)
[![Threads](https://img.shields.io/badge/Threads-000000?style=for-the-badge&logo=threads&logoColor=white)](https://www.threads.net/intent/post?text=Is%20iCloud%27s%20free%20tier%20still%205GB%3F%20Yes.%20https%3A%2F%2Fisicloud5gb.lol)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/company/isicloud5gb)
[![Discord](https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/B2H737b73S)
[![Telegram Bot](https://img.shields.io/badge/Telegram%20Bot-26A5E4?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/isicloud5gb_bot)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/digitalby/isicloud5gb.lol)
<a href="https://hosted.weblate.org/engage/isicloud5gb/">
<img src="https://hosted.weblate.org/widget/isicloud5gb/isicloud5gb-l10n/svg-badge.svg" alt="Translation status" />
</a>
[![GitHub Sponsor](https://img.shields.io/badge/GitHub-Sponsor-ea4aaa?style=for-the-badge&logo=githubsponsors&logoColor=white)](https://github.com/sponsors/digitalby)

[English](README.md) | [日本語](README.ja.md) | [Polski](README.pl.md) | **Русский**

## FAQ

**Q: Всё ещё 5 ГБ?**

A: Да.

**Q: А что сейчас?**

A: Всё ещё 5 ГБ.

**Q: Has it changed since the last time I checked?**

A: Нет. Всё ещё 5 ГБ.

**Q: Но ведь компания стоимостью $3 трлн наверняка может--**

A: 5 ГБ.

**Q: Google даёт 15 ГБ бесплатно.**

A: Это не вопрос. Но да — так и есть. Втрое больше, чем у Apple. Бесплатно.

**Q: Apple когда-нибудь его увеличит?**

A: Check back at [isicloud5gb.lol](https://isicloud5gb.lol). If the site is still up, the answer is still no.

## Тех. стек

This entire website has more meta tags than content, because the answer is one word. Feel free to overengineer it as much as you want.

## Контрибьюшн

Контрибьютить тут почти нечего. Ответ — *да*.

Чтобы наша вещательная миссия стала шире — вот список желаний:
- доступность
- локализация
- приспособления для дизайна

Пусть о страданиях узнает весь мир:
- откройте пулл-реквест сегодня
- поделитесь ссылкой в соцсетях
- отправьте ссылку в ваш семейный чат
- ask grandma if she ran out of iCloud storage today and keeps deleting those family archive pics that have never been backed up

But if Apple ever changes the answer to this question, open a PR and we'll shut the whole site down.

### Переводы

Help translate this site into your language via [Weblate](https://hosted.weblate.org/engage/isicloud5gb/).

## Лицензия

MIT.

Because unlike iCloud storage, this is actually *free*.
